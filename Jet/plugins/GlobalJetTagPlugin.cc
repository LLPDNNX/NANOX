    ...
    public:
        ...
        virtual void produce(edm::Event& event, const edm::EventSetup&) const
        {
            ...
            std::unique_ptr<std::vector<xtag::GlobalJetTagData>> output(
                new std::vector<xtag::GlobalJetTagData>(jetCollection->size())
            );

            for (unsigned int ijet = 0; ijet < jetCollection->size(); ++ijet)
            {
                const pat::Jet& jet = jetCollection->at(ijet);
                (*output)[ijet].jetPt = jet.pt();
                (*output)[ijet].jetEta = jet.eta();
            }
...
DEFINE_EDM_PLUGIN(xtag::XTagPluginFactory, xtag::GlobalJetTagDataPlugin, "GlobalJetTagDataPlugin");
